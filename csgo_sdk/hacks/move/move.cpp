...
namespace hacks {
	void c_move::handle(c_user_cmd* cmd) {
		if (g_cfg->m_move.m_bhop
	...
	void c_move::correct(c_user_cmd* cmd, qangle_t old_angles) {
		/* movement correction, u can improve this urself but again, this is for example */
		const auto move_type = globals::m_local->get_move_type();
		if (move_type == MOVE_TYPE_NOCLIP
			|| move_type == MOVE_TYPE_LADDER)
			return;

		vec3_t right;
		auto fwd = cmd->m_view_angles.vector(&right);
		...
		if (const auto div = right.y * fwd.x - right.x * fwd.y) {
			vec3_t old_right;
			auto old_fwd = old_angles.vector(&old_right);
			...
			static const auto cl_backspeed = interfaces::m_cvar_system->find_var(FNV1A("cl_backspeed"));
